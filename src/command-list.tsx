...
import { CheckCircle, Circle, AlertCircle, ExternalLink, RefreshCcw } from "lucide-react"
    ...
    <Alert className="mb-4">
      ...
      <AlertDescription className="flex justify-between space-y-2 flex-col">
        <span className="text-sm text-slate-700">
          Important! Please check the system requirements before installing Docker.
        </span>
        <a
...
export function CommandControl() {
  ...
  useEffect(() => {
    const checkSearxngState = () => {
      invoke<string>("get_searxng_state")
        .then((port) => {
          if (currentStepRef.current == 'start_docker' || currentStepRef.current == 'wait_docker') return;
          if (!port) {
            setStatus("idle")
    ...
    const listeners = [
      ...
      },
      {
        ...
        handler: (event: any) => {
          setCurrentStep("done_searxng")
          setStatus("done")
          setMessage("SearXNG is running on port: " + event.payload as string)
    ...
    const unlisteners = listeners.map(({ event, handler }) => listen(event, handler))
  ...
  const getStepIcon = (stepKey: Step) => {
    ...
    if (steps.findIndex((s) => s.key === stepKey) < steps.findIndex((s) => s.key === currentStep)) {
  ...
  const getStepDescription = (stepKey: Step) => {
    if (stepKey === "wait_docker" && currentStep === "wait_docker") {
      return <span className="text-xs text-muted-foreground">Timeout in {dockerStartTimeout}s</span>
    }
  ...
  return (
    <Card
      ...
      className="w-full mx-2 bg-background border-none shadow-none h-auto flex flex-col justify-center overflow-hidden max-h-48 rounded-none px-4"
    >
      <ScrollArea className="h-full" data-tauri-drag-region>
        <CardHeader data-tauri-drag-region>
          <CardTitle className="text-2xl font-bold text-primary">Docker & SearXNG Setup</CardTitle>
          <CardDescription className="text-muted-foreground text-xl">Run SearXNG</CardDescription>
        </CardHeader>
        <CardContent data-tauri-drag-region>
          
          {dockerInstalled === false ? (
            <div data-tauri-drag-region className="flex flex-col items-center space-y-4">
              <p className="text-sm text-red-500">Docker is not installed. Please install Docker to continue.</p>
          ...
          <Alert className="mb-4 flex  items-center">
            ...
            <div data-tauri-drag-region className="space-y-2">
              {steps.map((step, index) => (
                <div data-tauri-drag-region key={step.key} className="flex flex-col space-y-1">
                  <div data-tauri-drag-region className="flex items-center justify-between">
                    <div data-tauri-drag-region className="flex items-center space-x-2">
                      ...
                      <span
                        className={cn("text-sm", currentStep === step.key ? "font-medium" : "text-muted-foreground")}
                      >
                        {step.title}
                      </span>
                    </div>
        ...
        <CardFooter data-tauri-drag-region>
          {dockerInstalled !== false && (
            ...
            >
              {status !== "done" && (
                <>
                <span>{status === "idle" ? "Start Setup" : status === "error" ? "Try Again"  : "Setting up..."}</span>
