...
const MessageSources = ({ sources }: { sources: Document[] }) => {
    ...
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
        { sources.slice(0, showAllSources ? sources.length : 3).map((source, i) => (
          <a
        ...
        {sources.length > 3 && (
          ...
          >
            ...
            <Button onClick={()=>{setShowAllSources(prev=>!prev)}} asChild variant="ghost" size="sm" className="text-xs font-medium">
            <p className="text-xs text-black/50 dark:text-white/50">
              {!showAllSources ? `View ${sources.length - 3} more`: "Hide sources"}
              </p>
            </Button>
          </button>
        )}
