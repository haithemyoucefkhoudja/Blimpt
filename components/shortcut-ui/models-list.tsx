...
const ModelsList: React.FC<ModelsListProps> = ({
  ...
  const [newModel, setNewModel] = useState<Model>({
    id: "",
    name: "",
    provider: providers && providers[0] ? providers[0].name : "",
  ...
  return (
    <div className="w-full mx-auto p-4">
      ...
      <ul className="space-y-4">
        {models &&
          models.map((model) => (
            <li
              key={model.id}
              className="bg-background border rounded-lg p-4 flex items-center justify-between"
            >
              {editingModel === model.id ? (
                <div className="flex-1 space-y-2">
                  <Input
                    value={model.name}
                    onChange={(e) =>
                      handleModelChange(model.id, "name", e.target.value)
                    }
                    className="w-full"
                  />
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleModelChange(model.id, "provider", value)
                    }
                  >
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        handleModelChange(model.id, "isDeepThinking", checked)
                      }
                    />
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      checked={model.isImage}
                      onCheckedChange={(checked) =>
                        handleModelChange(model.id, "isImage", checked)
                      }
                    />
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2">
                      {model.isDeepThinking && (
                        <span className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded-full truncate">
                          Deep Thinking
                        </span>
                      )}
                      {model.isImage && (
      ...
      <div className="mt-6 space-y-2">
        ...
        <div className="flex items-center space-x-2">
          <Switch
            ...
            onCheckedChange={(checked) =>
              setNewModel({ ...newModel, isDeepThinking: checked })
            }
          />
          <span className="text-sm">Deep Thinking</span>
        </div>
      ...
      <div className="mt-6 space-y-2">
        ...
        ></ModelSelect>
        <Label>Choose DeepThinking Model:</Label>
        <ModelSelect
          selectedModel={config.selectedDeepThinkingModel}
